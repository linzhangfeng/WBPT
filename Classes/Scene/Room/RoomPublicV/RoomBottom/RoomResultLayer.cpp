...
bool RoomResultLayer::init()
{
    ...
    touchListener->onTouchBegan =CC_CALLBACK_2(RoomResultLayer::onTouchBegan,this);
    ...
    Text* titlelabel = Text::create("实时战绩", "AmericanTypewriter", 36);
...
void RoomResultLayer::getData(float dt)
{
    CCHttpPacket* loginPacket = CCHttpAgent::getInstance()->packets["room_result"];
    if (loginPacket && loginPacket->status != 0)
    {
        unschedule(schedule_selector(RoomResultLayer::getData));
        ...
        CCHttpAgent::getInstance()->packets["room_result"] = NULL;
        delete loginPacket;
    }
}

void RoomResultLayer::myInit(int rid)
{
    Json::Value json;
    json["roomId"] = rid;
    json["UserKey"] = ZJHModel::getInstance()->UserKey;
    
    CCHttpAgent::getInstance()->sendHttpPost(NULL,"duiju/integral/current", json.toStyledString(), "room_result");
    
    this->schedule(schedule_selector(RoomResultLayer::getData), 0.0f);
}

void RoomResultLayer::initListView(Json::Value &data)
{
    Size winSize = Director::getInstance()->getWinSize();
    Size contentSize = Size(winSize.width, 790);
    ...
    //我的战绩
    {
        ...
        Text *myTitle = Text::create("我的战绩", "AmericanTypewriter", 32);
        myTitle->setColor(numColor);
        myTitle->setPosition(Vec2(90, itemHeight * 0.5));
        myResult->addChild(myTitle);
        
        snprintf(buff, sizeof(buff), "%d", data["integral"].asInt());
    ...
    for (int i = 0; i < data["playRank"].size(); i++) {
        ...
        int userId = data["playRank"][i]["userId"].asInt();
        int integral = data["playRank"][i]["integral"].asInt();
        std::string userName = data["playRank"][i]["userName"].asString();
        std::string image = data["playRank"][i]["image"].asString();
...
bool RoomResultLayer::onTouchBegan(cocos2d::Touch *touch, cocos2d::Event *event)
{
