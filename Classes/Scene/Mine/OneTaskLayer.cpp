...
bool OneTaskLayer::init()
{
    ...
    CCHttpAgent::getInstance()->sendHttpPost(NULL,"task/taskList",json1.toStyledString(),"taskList1");
    this->schedule(schedule_selector(OneTaskLayer::postTask1), 0.0f);
    loading = Loading::startLoading(this, this->getContentSize()/2);
    
    return true;
}

void OneTaskLayer::postTask1(float dt)
{
    CCHttpPacket* packet = CCHttpAgent::getInstance()->packets["taskList1"];
    if (packet && packet->status != 0)
    {
        loading->removeFromParent();
        unschedule(schedule_selector(OneTaskLayer::postTask1));
        ...
        if (packet->resultIsOK())
        {
            ...
            log("任务列表1：＝＝＝ [%s]",data.toStyledString().c_str());
        }
        else
        {
        ...
        CCHttpAgent::getInstance()->packets["taskList1"] = NULL;
        delete packet;
    }
    
}

void OneTaskLayer::showView1(Json::Value list)
{
...
Layout *OneTaskLayer::createItem(Json::Value json, int index)
{
    ...
    switch (json["status"].asInt())
    {
        ...
        case 1:
            ...
            btn->addClickEventListener(CC_CALLBACK_1(OneTaskLayer::receiveOnFun, this));
            break;
        case 2:
            //已领取
...
void OneTaskLayer::receiveOnFun(Ref *pSender)
{
    if (ZJHModel::getInstance()->isYoukeLogin())
    {
        ZJHModel::getInstance()->gotoView(VIEW_LOAD);
