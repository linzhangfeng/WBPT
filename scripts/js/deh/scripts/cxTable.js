...
var CXTable = cc.Layer.extend({
    ...
    ctor:function () {
        ...
        cc.spriteFrameCache.addSpriteFrames("deh/res/img/card.plist");
        cc.spriteFrameCache.addSpriteFrames("public/res/img/bigface.plist");
        cc.spriteFrameCache.addSpriteFrames("deh/res/img/cardtype.plist");
        cc.spriteFrameCache.addSpriteFrames("deh/res/img/common.plist");

        ccs.armatureDataManager.addArmatureFileInfo("deh/res/effect/effectmaxuejia.ExportJson");
        ...
        CXModel.isOnVideo = ZJHModel.getInstance().isOnVideo;
        if ( CXModel.isOnVideo ) {
            log("===========here===here=========");
            this.videoBox = new CXVideo();
    ...
    init:function()
    {
        ...
        CXPlayers.publicBox = RoomPublic.create();
    ...
    test:function()
    {
        ...
        if(this.testCount == 3)
        {
            ...
            for(var k = 0;k<6;k++)
            {
                for(var j = 0;j<2;j++)
                {
                    CXModel.otherPlayCards[k].push(5);
                }
    ...
    initView:function()
    {
        this.uiNode.setLocalZOrder(120);
        ...
        CXCards.uiNode.setLocalZOrder(100);
        CXCards.init();
        // cc.log("ti2:"+((new Date).valueOf()-b1));
        // b1 = (new Date).valueOf();

        CXChips.uiNode =  this.getUI("ctip");
        CXChips.uiNode.setLocalZOrder(101);
        CXChips.init();
        ...
        CXModel.jzView.setPosition(cc.p(0,0));
        CXModel.jzView.callBackAction = this.jzCallBack;
        this.addChild( CXModel.jzView,130);
        CXModel.jzView.setVisible(false);

        //设置簸簸界面
        CXModel.boboView  = new CXBoboView();
        CXModel.boboView.setPosition(cc.p(0,0));
        this.addChild( CXModel.boboView,130);
        CXModel.boboView.setVisible(false);

        //扯牌
        CXModel.CmbinationView  = new CXCmbinationView();
        CXModel.CmbinationView.setPosition(cc.p(0,0));
        this.addChild( CXModel.CmbinationView,130);
    ...
    updateGame:function( dt)
    {
        var curTime = (new Date).valueOf()/1000;
         ...
         if( CXModel.reconnectTime!=-1)
            {
                CXModel.reconnectTime +=dt;

                if(CXModel.reconnectTime >3)
                {
                    CXModel.changeIP();
                    CXModel.connectNet();
                    return;
    ...
    showEndRoom:function(val)
    {
        ...
        this.roomEndBox.setCallback(function (str){
         ...
         if(this.roomEndBox.setDefaultTips){
            this.roomEndBox.setDefaultTips("若您超时未选择，则系统默认为\"同意解散\"");
        }
    },
//*************************** 按钮点击发送服务器处理 ************************************//
    //点击事件处理
 ...
 //*************************** 服务器返回信息处理 ************************************//
    ...
    handlerCMD:function(data,isDelay)
    {
        ...
        switch (cmd)
        {
            ...
            case NetCMD.SERVER_VIDEO_FIRST_HANDCARD:
                this.firstHandCards(val);
                break;
    ...
    handlerShowGameEndView:function(val)
    {
        ...
        if(CXModel.gameEndView != null)
        {
            CXModel.gameEndView.removeFromParent(true);
            CXModel.gameEndView = null;
        }

        var box = new CXGameEndView();
        box.setPosition(cc.p(0,0));
        box.initdata(val);
        this.addChild(box,200000);
    },
            ...
            for(var i = 0;i<playleng;i++)
            {
                ...
                if(d.hasOwnProperty("cards"))
                {
                    ...
                    else
                    {
                        if (CXModel.isOnVideo)
                        {
                            cardlong = CXModel.otherPlayCards[c_pos].length;
                        }else{
                    ...
                    for(var j = 0;j<lencards;j++)
                    {
                        ...
                        if(!CXModel.fastShow)
                        {
                            log("123456=====================发牌"+c_pos+"==="+index)
                            CXCards.showCards(c_pos,index,delay);//发牌
                        }
                        else
                        {
                            this.checkAllRoundFinish(0);
                        }
    ...
    gameReadyStart:function(val)
    {
        ...
        CXModel.state = CXState.READY;

        // CXTips.showLJXiu(false);
        CXChips.showLJXiu(false);
    ...
    ChepaiStart:function(seatids,flag)
    {
        ...
        if(flag)
        {
            ...
            CXChips.showTipWei(true);   //尾  wei_cards_name
        }
                ...
                if(len > 2)
                {
                    ...
                    if(!CXModel.isMySeatID(seatids[i]))
                    {
                        ...
                        else
                        {
                            ...
                            if (CXModel.isOnVideo)
                            {
                                CXCards.showCardValueAndPos_video(cpos,0,0,CXModel.otherPlayCards[cpos][2]);
                                CXCards.showCardValueAndPos_video(cpos,1,0,CXModel.otherPlayCards[cpos][3]);
                            }
                            else
                            {
        ...
        if(flag)
        {
            // CXTips.showTipTou(true);   //头 tou_cards_name
             CXChips.showTipTou(true);   //尾  wei_cards_name
        }
                    ...
                    if(!CXModel.isMySeatID(seatids[i]))
                    {
                        ...
                        if(_player.tou_cards_name == 0 || _player.tou_cards_name == 1)
                        {
                           if(CXModel.otherPlayCards[cpos].length > 3)
                           {
                            ...
                            if (CXModel.isOnVideo)
                            {
                                CXCards.showCardValueAndPos_video(cpos,2,0,CXModel.otherPlayCards[cpos][0]);
                                CXCards.showCardValueAndPos_video(cpos,3,0,CXModel.otherPlayCards[cpos][1]);
                            }
                            else
                            {
        ...
        if(this.Advancefp == true)
        {
            ...
            if(val.hasOwnProperty("chepai_seatid"))
            {
                ...
                for(var k = 0; k< lengths; k++)
                {
                   ...
                   if (CXModel.isOnVideo)
                   {
                        CXCards.reverseAllCards_video(_pos,delay);
                   }
                   else
                   {
            ...
            for(var i = 0;i<6;i++)
            {
                ...
                if(player != null && player.ready == 1)
                {
                    CXModel.allPlayerChips = CXModel.allPlayerChips + player.total_bet;
                    var pos = CXModel.getPosBySeatID(i);
                    CXChips.sendAllPlayerChipTpTable(pos,CXModel.allPlayerChips);
                }
...
//当前的下注操作
    showNext_bet_bc:function(val)
    {
        ...
        if(CXModel.isMySeatID(seait))
        {
            ...
            if(!CXModel.isOnVideo)
            {
                if(CXModel.fastShow)
               {
                    CXAction.showOutCardBtns_video();
               }
               else
               {
                    CXAction.showOutCardBtns();
 ...
 //下注结果的的返回
    sendBetSucc_bc:function(val)
    {
        ...
        if(CXModel.isMySeatID(seatid))
        {
            CXModel.myoptionfinish = true;
             ...
             if(!CXModel.isOnVideo)
             {
                if(CXModel.fastShow)
                {
                    CXAction.hideOutCardBtns_video();
                }
                else
                {
                    CXAction.hideOutCardBtns();
        ...
        nplayer.showSuccessAction();//chip背景恢复
    ...
    firstHandCards:function(val)
    {
        ...
        if(val.hasOwnProperty("players"))
        {
            ...
            for(var  i = 0;i <leng; i++)
            {
                ...
                var llen = d["cards"].length;
                for(var k = 0; k< llen;k++)
                {
                    var cardValue = d["cards"][k];
                    ...
                    if(CXModel.fastShow)
                    {
                        CXCards.showCardValue(c_pos,k,0,CXModel.otherPlayCards[c_pos][k]);
                    }
        ...
        if(CXCards.cardNums <= 0)
        {
            ...
            if(CXModel.isOnVideo)
            {
                 for(var i =0;i<6;i++)
                {
                    ...
                    if(num > 0)
                    {
                        CXCards.showCardValue(i,0,0,CXModel.otherPlayCards[i][0]);
                        CXCards.showCardValue(i,1,0,CXModel.otherPlayCards[i][1]);
                    }
                }
            }
            else
            {
                for(var i =0; i< length; i++)
                {
                    CXCards.showCardValue(pos,i,0,CXModel.myHandCard[i]);
                }
            }
    ...
    secondRound:function(val)
    {
        ...
        if(val.hasOwnProperty("players"))
        {
            ...
            for(var  i = 0;i <leng; i++)
            {
                ...
                {
                    CXCards.showCardValue(c_pos,2,0,CXModel.otherPlayCards[c_pos][2]);
                }
        ...
        if(CXCards.cardNums <= 0)
        {
            ...
            for(var i =0;i<6;i++)
            {
                ...
                if(CXModel.isOnVideo)
                {
                    if(num > 2)
                    {
                        CXCards.showCardValue(i,2,delay,CXModel.otherPlayCards[i][2]);
                    }
                }
                else
                {
   ...
   thirdRound:function(val)
    {
        ...
        if(val.hasOwnProperty("players"))
        {
            ...
            for(var  i = 0;i <leng; i++)
            {
                ...
                {
                    CXCards.showCardValue(c_pos,3,delay,CXModel.otherPlayCards[c_pos][3]);
                }
        ...
        if(CXCards.cardNums <= 0)
        {
            ...
            for(var i =0;i<6;i++)
            {
                ...
                if(CXModel.isOnVideo)
                {
                    if(num > 3)
                    {
                        CXCards.showCardValue(i,3,delay,CXModel.otherPlayCards[i][3]);
                    }
                }
                else
                {
...
//设置簸簸成功
    ...
    handlerTableInfoUC:function(val) {
        ...
        if (val.hasOwnProperty("teshu_select"))
        {
            CXModel.teshu_select = utils.getValInt(val, "teshu_select");
        }
        if (val.hasOwnProperty("dijiuwang_select"))
        {
            CXModel.dijiuwang_select = utils.getValInt(val, "dijiuwang_select");
        }

        if (val.hasOwnProperty("first_hand_select"))
        {
            CXModel.first_hand_select = utils.getValInt(val, "first_hand_select");
        }
        

        CXModel.stand_bobo = 15;
        ...
        if (val.hasOwnProperty("players")) {
            ...
            for (var i = 0; i < len; i++)
            {
                ...
                if(CXModel.state == CXState.CHEPAI || CXModel.state == CXState.SETTLEMENT )
                {
                    var num = Math.ceil(player.total_bet/CXModel.stand_bobo); 
                    CXModel.allPlayerChips = CXModel.allPlayerChips + player.total_bet; 
                    CXChips.sendBetToTable(pos,num,CXModel.allPlayerChips);
                }
        ...
        {
            CXAction.showSzBtn(true);
        }else{
            CXAction.showSzBtn(false);
        }
...
//---------------------------------------------------------------------------------------------------------
        ...
        CXTips.showGameTip(CXModel.teshu_select,CXModel.dijiuwang_select,CXModel.first_hand_select);    // gameTip
    },
    ...
    handlerReadySuccBC:function(val)
    {
        ...
        if(CXModel.isMyUID(uid))
        {
            if(CXModel.gameEndView != null)
            {
                CXModel.gameEndView.setVisible(false);
            }
    ...
    handlerPreready_Begin:function(val)
    {
        ...
        if(val.hasOwnProperty("game_nums"))
        {
            CXModel.curGame = val["game_nums"];
        }


        if(CXModel.gameEndView == null)
        {
            CXModel.gameEndView  = new CXBalanceView();
            CXModel.gameEndView.callBackAction = this.nextGameStart;
            CXModel.gameEndView.setPosition(cc.p(0,0));
            this.addChild(CXModel.gameEndView,130);
        }
        else
        {
            CXModel.gameEndView.setVisible(true);
        }

        CXModel.gameEndView.initdata();
    ...
    handlerGamePrereadyBC:function(val)
    {
        ...
        if(CXModel.isMyInTable())
        {
            // CXAction.showReadyBtn(true);  //广播准备开始按钮显示 
        }
    ...
    handlerGameStartBC:function(val)
    {
        ...
        if(CXModel.gameEndView != null)
        {
            CXModel.gameEndView.setVisible(false);   //隐藏小结
        }
        ...
        else if(estate == 1)   //－－－－－－－－－－－－－－－－－－弃－－－－－－－－－－－－－－
        {
            ...
            if(val.hasOwnProperty("players"))
            {
                ...
                for(var i = 0; i< leng; i++)
                {
                    ...
                    if(d.hasOwnProperty("is_diupi"))
                    {
                        // var sendpi = d["pi_money"];
                        var flag = utils.getValBool(d, "is_diupi");
                        if(flag)
                        {
                            var sendpi = d["win_lose"];
                            sendpi = -sendpi;
                        ...
                        {
                            if(d.hasOwnProperty("win_lose"))
                            {
                                var lostmoney = d["win_lose"];
                                if(lostmoney == 0)
                                {
