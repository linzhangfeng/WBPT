...
var CXCards =
{
    ...
    createCard:function()
    {
        for(var i = 0;i < this.player_num; i++)
        {
            for(var j = 0;j < this.player_cards; j++)
            {
                ...
                if(i == 1 || i == 2)
                {
                    card.setLocalZOrder(parseInt(CardTagUM.ALLCARDBEGIN + this.player_cards*i + (4-j) ));
                }
                else
                {
    ...
    showCards:function(pos,index,delay)
    {
        ...
        (function(card,spos,self){
                        ...
                        var startpos = utils.getFpStartPos();
                        ...
                        var romo = cc.RotateTo(0.2,0);
                        var cmoved = cc.moveTo(0.2,spos);
                        var squa = cc.Spawn(romo,cmoved);
                        ...
                        card.runAction(cc.sequence(carddelay,show_card,squa,cardMove,card_sound,cardFadeIn,card_cend));
         }) (card,beginpos,this);
    },
    ...
    addChePaiFinish:function(pos)
    {
        ...
        if(ft == null)
        {
            ...
            var players = utils.findNode(this.uiNode.getParent(),"players");
            var posi = players.getChildByName("player"+pos).getPosition();
     ...
     showCardValueAndPos:function(pos,index,delay,value)
    {
     ...
     showCardValueAndPos_video:function(pos,index,delay,value)
    {
        var card = this.uiNode.getChildByTag(CardTagUM.ALLCARDBEGIN + this.player_cards*pos+index);
        var players = utils.findNode(this.uiNode.getParent(),"players");
        var position = utils.getSplitCardPos(players,pos,index);
    ...
    showCardType:function(pos,index,type,flag)
    {
        ...
        if(index == 0 || index == 1 )
        {
            ...
            if(pos == 1 || pos == 2)
            {
                if(index == 0)
                {
                    cc.pAddIn(position,cc.p(-10,-25));
                }else
                {
                    cc.pAddIn(position,cc.p(-10,-35));
                }
            }
            else
            {
            ...
            position = utils.getCardPos(players,pos,1);
        ...
        var cardType = null;
        cardType = cc.Sprite(utils.spriteFrame("CXCardType/"+type+".png"));
    ...
    reverseAllCards_video:function(pos,delay)
    {
        var players = utils.findNode(this.uiNode.getParent(),"players");
        var length = CXModel.otherPlayCards[pos].length;
        for(var i = 0;i < length;i++)
        {
            var card = this.uiNode.getChildByTag(CardTagUM.ALLCARDBEGIN + this.player_cards*pos + i);
            if(card != null)
            {
                var position = utils.getSplitCardPos(players,pos,i);
                card.setVisible(true);
