var WZDNBigfaceBox = ccui.Layout.extend(
    {
        pos:0,
        ctor: function ()
        {
        ...
        addUIClickEvt:function(node,key)
        {
            ...
            if(ui) {
                ui.addClickEventListener(function (sender, evt) {
                    ...
                    if(key == "btn7")
                    {
                        WZDNPlayers.publicBox.showPlayerList();
                    }
                    else
                    {
                       var id = Number(key.substr(3,1));
                        if( utils.getTimeStamp() - WZDNTips.bigfaceTime >2000)
                        {
                            WZDNModel.sendEmotion(self.pos,id);
                            WZDNTips.bigfaceTime = utils.getTimeStamp();
                        }
                        var b = WZDNPlayers.getPosition(0);
                        var t  =WZDNPlayers.getPosition(self.pos);
                        WZDNTips.showBigface(b,t,id,WZDNPlayers.getSex(0),self.pos);
                    }
                    self.removeFromParent(true);
                });
            }
...
var WZDNBalanceItem = ccui.Layout.extend(
    {
        ctor: function () {
            this._super();
        ...
        show:function(player)
        {
                ...
                var m_pAvatar = utils.createCircleAvatar(player.avatar, "wznn/res/pic/p_bg.png", "wznn/res/pic/p_bg.png", cc.size(92,92));
...
var WZDNBalanceView = cc.Layer.extend(
    {
        uiJson: null,
        uiNode: null,
...
var WZDNGameEndItem = ccui.Layout.extend(
    {
        ctor: function () {
            this._super();
        ...
        show:function(data,flag)
        {
            ...
            this.m_pAvatar = utils.createCircleAvatar(avrt, "wznn/res/pic/p_bg.png", "wznn/res/pic/p_bg.png", cc.size(92,92));
...
//游戏结束 结算界面
var WZDNGameEndView = cc.Layer.extend(
    {
        uiJson: null,
        uiNode: null,
        ...
        ctor: function ()
        {
            ...
            this.getUI("btn_share").addClickEventListener(function (sender, evt) {
                ...
                WZDNPlayers.publicBox.setVisible(false);
                ...
                renderTexture.saveToFile("record.jpg", cc.IMAGE_FORMAT_JPEG, true,function(text,path){
                    ...
                    WZDNPlayers.publicBox.setVisible(true);
        ...
        initdata:function(val)
        {
            ...
            this.getUI("roomnum").setString(WZDNModel.roomData["gameRoom"]["code"]);
            this.getUI("roomtime_s").setString("开始:"+utils.getTime(WZDNModel.roomData["gameRoom"]["createTime"]));
            ...
            if(val.hasOwnProperty("players"))
             ...
             if(val.hasOwnProperty("players"))
             {
                ...
                for(var i = 0;i < len; i++)
                {
                    ...
                    var data = val["players"][i];
                    var item  = new WZDNGameEndItem();
                    if(maxScoreIndex == i)
                    {
                        item.show(data,true);
                    }
