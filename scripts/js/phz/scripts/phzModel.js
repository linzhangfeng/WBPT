...
//*********************动态数据处理************************///
    ...
    checkCMD:function(cmd1,len)
    {
        var cmd = 0;
        switch (cmd1)
        {
            case NetCMD.SERVER_PHZ_PRE_OPERATE_RES:
                cmd = NetCMD.CLIENT_PHZ_PRE_OPERATE_REQ;
                break;
            //操作
        ...
        if( this.msData.hasOwnProperty(cmd) )
        {
            ...
            if(dis>1000)
            {
                ...
                cc.log("dis cmd:"+cmd+" ms:"+dis+" size:"+len);
            }
        }
    },
    clearCMD:function()
    ...
    //发送出牌请求
    sendOutCard:function(card,row,col)
    {
        var  d = packet;
        d.data = {};
    ...
    sendOffline:function()
    {
        var  d = packet;
        d.data = {};
        d.data["cmd"]  =NetCMD.CLIENT_PHZ_OFFLINE_INFO_REQ;
        this.sendData(d.toStr());
    },
