var PHZBigfaceBox = ccui.Layout.extend(
    {
        ...
        ctor: function ()
        {
            ...
            this.uiNode[2].setPosition(60,460);
...
var PHZTipBox = cc.Layer.extend(
    {
        ...
        ctor: function () {
            this._super();
            this.uiJson = ccs.load(PackName+"/res/tipBox.json");
            this.uiNode = this.uiJson.node;
            this.addChild(this.uiNode);
            ...
            this.getUI("sureBtn").addClickEventListener(function (sender, evt) {
                PHZModel.sendOutCard(this.card.value, this.card.row, this.card.col);
                PHZCards.preOutMyCard(this.card);
                self.removeFromParent();
            }.bind(this));
            return true;
        },
...
var PHZGuoHuBox = cc.Layer.extend(
    {
        uiJson: null,
        uiNode: null,
        ctor: function () {
            this._super();
            this.uiJson = ccs.load(PackName+"/res/boxGuoHu.json");
            this.uiNode = this.uiJson.node;
            this.addChild(this.uiNode);
            ...
            this.getUI("sureBtn").addClickEventListener(function (sender, evt) {
                if(self.ack==PHZAck.ACK_NULL)
                {
                    PHZModel.sendAck(PHZAck.ACK_NULL,PHZChi.CK_NULL);
                }
                else if(self.ack==PHZAck.ACK_PENG)
                {
                    if(!PHZModel.ackAble)
                    {
                        PHZModel.sendPreAck(self.ack);
                    }
                    else
                    {
                ...
                else  if(self.ack==PHZAck.ACK_CHI)
                {
        ...
        show:function(ack,box)
        {
            this.chiBox = box;
            this.ack = ack;
            if(this.ack==PHZAck.ACK_PENG)
            {
                this.getUI("txt").setString("选择碰后视为放弃胡牌，确定碰吗？");
            }
            else if(this.ack==PHZAck.ACK_NULL)
            {
                this.getUI("txt").setString("选择过后视为放弃胡牌，确定过吗？");
            }
            else  if(this.ack==PHZAck.ACK_CHI)
            {
                this.getUI("txt").setString("选择吃后视为放弃胡牌，确定吃吗？");
            }
        }
