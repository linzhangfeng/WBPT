...
var PHZCards =
{
    ...
    checkOut:function(card,isV)
    {
        if(PHZModel.endScore>=0)
        {
            for(var i =1;i<3;++i)
            {
                for(var j in PHZCards.showCardsVal[i])
                {
                    var kind = PHZCards.showCardsVal[i][j]["weave_kind"];
                    if(kind ==PHZAck.ACK_WEI &&  PHZCards.showCardsVal[i][j]["card_list"][0] == card.value)
                    {
    ...
    moveHandCards:function(c,isB)
    {
        ...
        for (var i in this.handCardsVal) {
            ...
            for (var j in cards) {
                ...
                (function(card,p){
                    card.ableTouch=false;
                    card.runAction(cc.sequence(cc.moveTo(0.2,p),cc.callFunc(function(){
                        card.setClick(false);
                        card.ableTouch=true;
                    })));
                })(card,poses[i][j]);
    ...
    showDunCards: function (num) {
        ...
        this.getUI("dunNum").setPositionY(this.dunPos.y-1*(19-num));
    },
    ...
    showHandCards: function () {
        ...
        if(this.handCardsVal ==undefined)
        {
            this.handCardsVal =[];
        }
        var poses = this.getHandCardsPos(0, this.handCardsVal.length, 4);
    ...
    showShowCards: function (pos) {
        for (var i  in  this.showCards[pos]) {
            this.showCards[pos][i].reset();
        }
        this.showCards[pos] = [];

        var isMy = pos == 0 && PHZModel.isMyInTable();

        var poses = this.getShowCardsPos(pos, 0, this.showCardsVal[pos].length, 4);
        var self = this;
        var func = function (i, j, cardPos,card_list) {
            var card = self.newCard1();
            card.setPosition(cardPos);
            card.pos = cardPos;
            card.setVisible(true);
            self.showCards[pos].push(card);
            if (type == PHZAck.ACK_WEI || type == PHZAck.ACK_TI || type == PHZAck.ACK_CW) {
                if ((card_list[0] != INVAIL_CARD || type == PHZAck.ACK_TI || type == PHZAck.ACK_CW) && j == count - 1) {
                    card.setState(0);
        ...
        if (pos != 1) {
            for (var i in this.showCardsVal[pos]) {
                ...
                for (var j  in weaves["card_list"]) {
                    func(i, j, poses[i][j],weaves["card_list"]);
                }
            }
        }
        else {
            var len = this.showCardsVal[pos].length;
            for (var i in this.showCardsVal[pos]) {
                var weaves = this.showCardsVal[pos][i];
                var type = weaves["weave_kind"];
                var count = weaves["card_list"].length;
                for (var j  in weaves["card_list"]) {
                    func(i, j, poses[len - 1 - i][j],weaves["card_list"]);
                }
            }
        }
    },
    showAckCardsBack: function (pos,val) {
        var poses = [cc.p(800, 400), cc.p(900, 330), cc.p(380, 330)];
        var card = null;
        for (var i = 0; i < 3; ++i) {
                ...
                if(cs[z] ==i)
                {
                    for (var j = 0; j < this.showCardsVal[pos][i]["card_list"].length; ++j)
                    {
                        ...
                        if (card && (card.value == c.value || (card.state == c.state && c.state == 1))) {
                            p1 =utils.addPP( poses[pos],cc.p(0,40*j));
                        }
                        c.setPosition(utils.addPP( poses[pos],cc.p(0,40*j)));
                        c.stopAllActions();
    ...
    getHistoryCardsPos: function (pos, sxl, xl) {
        ...
        for (var i = sxl; i < xl; ++i) {
            ret[i] = cc.p(start.x + 40 * i, start.y);
        }
        return ret;
    },
    ...
    pushChiCards: function (val) {
        this.chiVal = val;
         for(var i in  this.chiVal)
         {
         var data = this.chiVal[i];
    ...
    //  添加展示区非吃牌
    pushShowCards1: function (pos, card, type, isNoHand) {
    ...
    showPublicCard:function(pos,val)
    {
        ...
        card.runAction(cc.sequence(cc.spawn(cc.scaleTo(0.2,1),cc.moveTo(0.2,p)),cc.delayTime(1),cc.moveTo(0.5,p2),cc.hide()));
    },
