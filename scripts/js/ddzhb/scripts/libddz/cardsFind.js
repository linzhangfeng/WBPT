
var libddz = libddz?libddz:{};

libddz.cardsFind={
    results:[],

    splitRocket:true,
    clear:function()
    {
        this.results = [];
    },
    create:function()
    {
        var ret =utils.clone(libddz.cardsFind);
        ret.clear();
        return ret;
    },
    //查找targetCards中大于tempCards的牌
    find:function(tempCardsParse,tempCards,targetCards)
    {
        this.clear();
        if (tempCardsParse.type != libddz.CardsType.CARD_TYPE_ERROR)
        {
            if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_ONE){
                this.findOne(tempCardsParse, tempCards, targetCards);
            }
            else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_TWO){
                this.findTwo(tempCardsParse, tempCards, targetCards);
            }
            else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_THREE){
                this.findThree(tempCardsParse, tempCards, targetCards);
            }
            else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_ONELINE){
                this.findOneLine(tempCardsParse, tempCards, targetCards);
            }
            else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_TWOLINE){
                this.findTwoLine(tempCardsParse, tempCards, targetCards);
            }
            else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_THREELINE){
                this.findThreeLine(tempCardsParse, tempCards, targetCards);
            }
            else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_THREEWITHONE){
                this.findThreeWithOne(tempCardsParse, tempCards, targetCards);
            }
            else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_PLANEWITHONE){
                this.findPlaneWithOne(tempCardsParse, tempCards, targetCards);
            }

            if (tempCardsParse.type < libddz.CardsType.CARD_TYPE_SOFT_BOMB){
                this.findSoftBomb(tempCardsParse, tempCards, targetCards);
                this.findHardBomb(tempCardsParse, tempCards, targetCards);
                this.findRocket(tempCardsParse, tempCards, targetCards);
                this.findFiveBomb(tempCardsParse, tempCards, targetCards);
                this.findPlaneBomb(tempCardsParse, tempCards, targetCards);
                this.findPlaneLaiziBomb(tempCardsParse, tempCards, targetCards);
            }
            else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_SOFT_BOMB){
                this.findHardBomb(tempCardsParse, tempCards, targetCards);
                this.findRocket(tempCardsParse, tempCards, targetCards);
                this.findPlaneBomb(tempCardsParse, tempCards, targetCards);
            }
            else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_HARD_BOMB){
                this.findHardBomb(tempCardsParse, tempCards, targetCards);
                this.findRocket(tempCardsParse, tempCards, targetCards);
                this.findFiveBomb(tempCardsParse, tempCards, targetCards);
                this.findPlaneBomb(tempCardsParse, tempCards, targetCards);
                this.findPlaneLaiziBomb(tempCardsParse, tempCards, targetCards);
            }
            else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_ROCKET) {
                this.findFiveBomb(tempCardsParse, tempCards, targetCards);
                this.findPlaneBomb(tempCardsParse, tempCards, targetCards);
                this.findPlaneLaiziBomb(tempCardsParse, tempCards, targetCards);
            }
            else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_FIVE_BOMB) {
                this.findPlaneBomb(tempCardsParse, tempCards, targetCards);
            }
             else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_PLANE_BOMB) {
                this.findPlaneBomb(tempCardsParse, tempCards, targetCards);
                this.findPlaneLaiziBomb(tempCardsParse, tempCards, targetCards);
             }
             else if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_PLANE_LAIZI_BOMB) {
                this.findPlaneBomb(tempCardsParse, tempCards, targetCards);
             }
        }

        return 0;
    },
    //查找tempCards
    findOne:function(tempCardsParse,tempCards,targetCards)
    {
        var isSplit = true;
        if(targetCards.line1.length>=2)
        {
            targetCards.line1.sort(function(v1,v2){
                return libddz.card.compare(v1,v2);
            });

            if(targetCards.line1[targetCards.line1.length-2].face == 16&&!this.splitRocket)
            {
                isSplit = false;
            }

        }
        for(var i in targetCards.line1)
        {
            if(targetCards.line1[i].face > tempCardsParse.face && !(!isSplit&&targetCards.line1[i].face>=16))
            {
                var d =[];
                d.push(targetCards.line1[i]);
                this.results.push(d);
            }
        }
    },
    //查找tempCards对子
    findTwo:function(tempCardsParse,tempCards,targetCards)
    {
        for (var i = 0; i < targetCards.line2.length; i += 2)
        {
            if (targetCards.line2[i].face > tempCardsParse.face)
            {
                var d =[];
                d.push(targetCards.line2[i]);
                d.push(targetCards.line2[i+1]);
                this.results.push(d);
            }
        }
        if(targetCards.laizi)
        {
            for (var i = 0; i < targetCards.line1.length; i += 1)
            {
                if (targetCards.line1[i].face > tempCardsParse.face)
                {
                    var d =[];
                    d.push(targetCards.line1[i]);

                    var face = targetCards.line1[i].face;
                    if(  face >13)
                    {
                        face -=  13;
                    }
                    var c =libddz.card.create(0x40|face);
                    d.push(c);
                    this.results.push(d);
                }
            }
        }
    },
    //查找tempCards3张
    findThree:function(tempCardsParse,tempCards,targetCards)
    {
        for (var i = 0; i < targetCards.card3.length; i += 3)
        {
            if (targetCards.card3[i].face > tempCardsParse.face)
            {
                var d =[];
                d.push(targetCards.card3[i]);
                d.push(targetCards.card3[i+1]);
                d.push(targetCards.card3[i+2]);
                this.results.push(d);
            }
        }
        if(targetCards.laizi)
        {
            for (var i = 0; i < targetCards.line2.length; i += 2)
            {
                if (targetCards.line2[i].face > tempCardsParse.face)
                {
                    var d =[];
                    d.push(targetCards.line2[i]);
                    d.push(targetCards.line2[i+1]);
                    var face = targetCards.line2[i].face;
                    if(  face >13)
                    {
                        face -=  13;
                    }
                    var c =libddz.card.create(0x40|face);
                    d.push(c);
                    this.results.push(d);
                }
            }
        }
    },
    //查找单顺
    findOneLine:function(tempCardsParse,tempCards,targetCards)
    {
        if(targetCards.len<5)
        {
            return;
        }
        var count = targetCards.line1.length - tempCards.line1.length;
        for (var i = 0; i <= count; i++)
        {
            if (targetCards.line1[i].face > tempCardsParse.face)
            {
                var end = i + tempCardsParse.len;
                if (tempCardsParse.checkLine(targetCards.line1, 1, i, end))
                {
                    var d =[];
                    for (var j = i; j < end; j++)
                    {
                        d.push(targetCards.line1[j]);
                    }
                    this.results.push(d);
                }
            }
        }

        if(targetCards.laizi)
        {
            for (var i = 0; i <= count+1; i++)
            {
                if (targetCards.line1[i].face > tempCardsParse.face)
                {
                    var end = i + tempCardsParse.len-1;
                    var cs = targetCards.line1.slice(i,end);
                    var temp = cs[0].value;

                    var j =i+1;
                    for( ;j <end;++j)
                    {
                        var dis = targetCards.line1[j].face -temp;

                        if (dis == 1)
                        {
                            temp = targetCards.line1[j];
                        }
                        else
                        {
                            if(dis == 2)
                            {
                                var face = temp+1;
                                if(  face >13)
                                {
                                    face -=  13;
                                }
                                var c =libddz.card.create(0x40|face);
                                cs.push(c);

                                if (tempCardsParse.checkLine(cs, 1)) {
                                    this.results.push(cs);
                                }
                            }
                            break;
                        }
                    }
                    if (j == end-1)
                    {
                        var face =temp+1;
                        if(  face >13)
                        {
                            face -=  13;
                        }
                        var c =libddz.card.create(0x40|face);
                        cs.push(c);
                        this.results.push(cs);
                    }
                }
            }
        }
    },
    //查找双顺
    findTwoLine:function(tempCardsParse,tempCards,targetCards)
    {
        if(targetCards.len<6)
        {
            return;
        }
        var lines = utils.clone(targetCards.line2);
        if(targetCards.laizi)
        {
            for(var i = 0;i<targetCards.card1.length;++i)
            {
                if (targetCards.card1[i].face > tempCardsParse.face)
                {
                    var face = targetCards.card1[i].face;
                    var c =libddz.card.create(0x40|face);
                    lines.push(c);
                    c =libddz.card.create(targetCards.card1[i].value);
                    lines.push(c);
                }
            }
        }
        var count = lines.length - tempCards.line2.length;
        for (var i = 0; i <= count; i+=2)
        {
            if (lines[i].face > tempCardsParse.face)
            {
                var end = i + tempCardsParse.len;
                if (tempCardsParse.checkLine(lines, 2, i, end))
                {
                    var lz = 0;
                    for(var j = i;j<end;j++)
                    {
                        if(lines[j].suit == libddz.Suit.LAIZI)
                        {
                            lz++;
                        }
                    }
                    if(lz >1)
                    {
                        continue;
                    }
                    var d =[];
                    for (var j = i; j < end; j++)
                    {
                        d.push(lines[j]);
                    }
                    this.endLaiZi(d);
                    this.results.push(d);
                }
            }
        }

    },
    //查找三顺
    findThreeLine:function(tempCardsParse,tempCards,targetCards)
    {
        var lines = utils.clone(targetCards.line3);
        if(targetCards.laizi)
        {
            for(var i = 0;i<targetCards.card2.length;i+=2)
            {
                if (targetCards.card2[i].face > tempCardsParse.face)
                {
                    var face = targetCards.card2[i].face;
                    var c =libddz.card.create(0x40|face);
                    lines.push(c);
                    c =libddz.card.create(targetCards.card2[i].value);
                    lines.push(c);
                    c =libddz.card.create(targetCards.card2[i+1].value);
                    lines.push(c);
                }
            }
        }
        var count = lines.length - tempCards.line3.length;
        for (var i = 0; i <= count; i+=2)
        {
            if (lines[i].face > tempCardsParse.face)
            {
                var end = i + tempCardsParse.len;
                if (tempCardsParse.checkLine(lines, 2, i, end))
                {
                    var lz = 0;
                    for(var j = i;j<end;j++)
                    {
                        if(lines[j].suit == libddz.Suit.LAIZI)
                        {
                            lz++;
                        }
                    }
                    if(lz >1)
                    {
                        continue;
                    }
                    var d =[];
                    for (var j = i; j < end; j++)
                    {
                        d.push(lines[j]);
                    }
                    this.endLaiZi(d);
                    this.results.push(d);
                }
            }
        }

    },
    //查找三带1
    findThreeWithOne:function(tempCardsParse,tempCards,targetCards)
    {
        if (targetCards.len < 4)
        {
            return;
        }

        var lines = utils.clone(targetCards.line3);
        if(targetCards.laizi)
        {
            for(var i = 0;i<targetCards.card2.length;i+=2)
            {
                if (targetCards.card2[i].face > tempCardsParse.face)
                {
                    var face = targetCards.card2[i].face;
                    var c =libddz.card.create(0x40|face);
                    lines.push(c);
                    c =libddz.card.create(targetCards.card2[i].value);
                    lines.push(c);
                    c =libddz.card.create(targetCards.card2[i+1].value);
                    lines.push(c);
                }
            }
        }
        for (var i = 0; i < lines.length; i += 3)
        {
            if (lines[i].face > tempCardsParse.face)
            {
                var d =[];
                d.push(lines[i]);
                d.push(lines[i+1]);
                d.push(lines[i+2]);
                if (targetCards.card1.length > 0)
                {
                    d.push(targetCards.card1[0]);
                }
                else
                {
                    for (var j = 0; j < targetCards.line1.length; j++)
                    {
                        if (targetCards.line1[j].face != d[0].face) {
                            d.push(targetCards.line1[j]);
                            break;
                        }
                    }
                }
                this.endLaiZi(d);
                this.results.push(d);
            }
        }

    },
    //查找三带1飞机
    findPlaneWithOne:function(tempCardsParse,tempCards,targetCards)
    {
        if (targetCards.len < 8)
        {
            return;
        }

        var lines = utils.clone(targetCards.line3);
        if(targetCards.laizi)
        {
            for(var i = 0;i<targetCards.card2.length;i+=2)
            {
                if (targetCards.card2[i].face > tempCardsParse.face)
                {
                    var face = targetCards.card2[i].face;
                    var c =libddz.card.create(0x40|face);
                    lines.push(c);
                    c =libddz.card.create(targetCards.card2[i].value);
                    lines.push(c);
                    c =libddz.card.create(targetCards.card2[i+1].value);
                    lines.push(c);
                }
            }
        }
        var count = lines.length - tempCards.line3.length;
        for (var i = 0; i <= count; i += 3) {
            if (lines[i].face > tempCardsParse.face) {
                var end = i + tempCards.card3.length;
                if (tempCardsParse.checkLine(lines, 3, i, end)) {

                    var lz = 0;
                    for(var j = i;j<end;j++)
                    {
                        if(lines[j].suit == libddz.Suit.LAIZI)
                        {
                            lz++;
                        }
                    }
                    if(lz >1)
                    {
                        continue;
                    }

                    var cards = [];
                    for (var j = i; j < end; j++) {
                        cards.push(lines[j]);
                    }
                    for (var j = 0; j < targetCards.card1.length; j++) {
                        cards.push(targetCards.card1[j]);
                        if (cards.length == tempCardsParse.len) {
                            break;
                        }
                    }

                    if (cards.length == tempCardsParse.len) {
                        this.endLaiZi(cards);
                        this.results.push(cards);
                        continue;
                    }

                    var flag = 0;
                    for (var j = 0; j < targetCards.line1.length; j++) {
                        flag = 0;
                        for (var k = 0; k < cards.length; k++) {
                            if (cards[k].face == targetCards.line1[j].face) {
                                flag = 1;
                                break;
                            }
                        }

                        if (flag == 1) {
                            continue;
                        }

                        cards.push(targetCards.line1[j]);
                        if (cards.length == tempCardsParse.len) {
                            break;
                        }
                    }

                    if (cards.length == tempCardsParse.len) {
                        this.endLaiZi(cards);
                        this.results.push(cards);
                        continue;
                    }
                }
            }
        }
    },

    //查找炸弹
    findSoftBomb:function(tempCardsParse,tempCards,targetCards)
    {
        if(!targetCards.laizi)
        {
            return;
        }
        for (var i = 0; i < targetCards.line3.length; i += 3)
        {
            var cards=[];
            cards.push(targetCards.line3[i]);
            cards.push(targetCards.line3[i + 1]);
            cards.push(targetCards.line3[i + 2]);
            var face = targetCards.line3[i].face;
            if(  face >13)
            {
                face -=  13;
            }
            var c =libddz.card.create(0x40|face);
            cards.push(c);
            this.results.push(cards);
        }
    },

    //查找炸弹
    findHardBomb:function(tempCardsParse,tempCards,targetCards)
    {
        for (var i = 0; i < targetCards.card4.length; i += 4)
        {
            if ((tempCardsParse.type == libddz.CardsType.CARD_TYPE_HARD_BOMB&&targetCards.card4[i].face > tempCardsParse.face)
            ||tempCardsParse.type< libddz.CardsType.CARD_TYPE_HARD_BOMB
            )
            {
                var cards=[];
                cards.push(targetCards.card4[i]);
                cards.push(targetCards.card4[i + 1]);
                cards.push(targetCards.card4[i + 2]);
                cards.push(targetCards.card4[i + 3]);
                this.results.push(cards);
            }
        }
    },

    //查找火箭
    findRocket:function(tempCardsParse,tempCards,targetCards)
    {
        var len = targetCards.card1.length;
        if (len >= 2)
        {
            if (targetCards.card1[len - 2].face == 16
                && targetCards.card1[len - 1].face == 17)
            {
                var cards=[];
                cards.push(targetCards.card1[len - 2]);
                cards.push(targetCards.card1[len - 1]);
                this.results.push(cards);
            }
        }
    },

    //查找5炸
    findFiveBomb:function(tempCardsParse,tempCards,targetCards)
    {
        if(!targetCards.laizi)
        {
            return;
        }
        for (var i = 0; i < targetCards.card4.length; i += 4)
        {
                var cards=[];
                cards.push(targetCards.card4[i]);
                cards.push(targetCards.card4[i + 1]);
                cards.push(targetCards.card4[i + 2]);
                cards.push(targetCards.card4[i + 3]);

            var c =libddz.card.create(0x4f);
            cards.push(c);
                this.results.push(cards);
        }
    },
    //查找飞机炸
    findPlaneBomb:function(tempCardsParse,tempCards,targetCards)
    {
        if (targetCards.len < 8)
        {
            return;
        }
        for (var i = 0; i < targetCards.card4.length; i += 4)
        {
            for (var j = i+8;j < targetCards.card4.length; j += 4) {

                if (tempCardsParse.checkLine(targetCards.card4, 4, i, j)) {
                    var cards = [];
                    for(var z=i;z<j;++j)
                    {
                        cards.push(targetCards.card4[z]);
                    }
                    this.results.push(cards);
                }
            }
        }
        if(tempCardsParse.type  == libddz.CardsType.CARD_TYPE_PLANE_BOMB)
        {
            for(var i =0;i< this.results.length;)
            {
                if(this.results[i].length < tempCardsParse.len)
                {
                    this.results.splice(i,1);
                    continue;
                }

                if(this.results[i].length == tempCardsParse.len&&this.results[i][0].face <=  tempCardsParse.face)
                {
                    this.results.splice(i,1);
                    continue;
                }
                ++i;
            }
        }
        else if(tempCardsParse.type  == libddz.CardsType.CARD_TYPE_PLANE_LAIZI_BOMB)
        {
            for(var i =0;i< this.results.length;)
            {
                if(this.results[i].length < tempCardsParse.len-1)
                {
                    this.results.splice(i,1);
                    continue;
                }

                if(this.results[i].length == tempCardsParse.len-1&&this.results[i][0].face <=  tempCardsParse.face)
                {
                    this.results.splice(i,1);
                    continue;
                }
                ++i;
            }
        }
    },
    //查找飞机癞子炸
    findPlaneLaiziBomb:function(tempCardsParse,tempCards,targetCards)
    {
        if(!targetCards.laizi)
        {
            return;
        }
        if (targetCards.len < 8)
        {
            return;
        }
        for (var i = 0; i < targetCards.card4.length; i += 4)
        {
            for (var j = i+8;j < targetCards.card4.length; j += 4) {

                if (tempCardsParse.checkLine(targetCards.card4, 4, i, j)) {
                    var cards = [];
                    for(var z=i;z<j;++j)
                    {
                        cards.push(targetCards.card4[z]);
                    }
                    var c =libddz.card.create(0x4f);
                    cards.push(c);
                    this.results.push(cards);
                }
            }
        }
        if(tempCardsParse.type  == libddz.CardsType.CARD_TYPE_PLANE_BOMB)
        {
            for(var i =0;i< this.results.length;)
            {
                if(this.results[i].length-1 < tempCardsParse.len)
                {
                    this.results.splice(i,1);
                    continue;
                }
                ++i;
            }
        }
    },
    //提示比tempCs大的牌 ,返回提示数量
    tip:function(tempCs,targetCs)
    {
        if (tempCs.length == 0)
        {
            return -1;
        }

        if (targetCs.length == 0)
        {
            return -2;
        }

        this.clear();

        var tempCards = libddz.cards.create(tempCs);
        var tempCardsParse = libddz.cardsParse.create(tempCards);
        if (tempCardsParse.type == libddz.CardsType.CARD_TYPE_ERROR)
        {
            return -1;
        }

        var targetCards = libddz.cards.create(targetCs);

        this.find(tempCardsParse, tempCards, targetCards);

        return this.results.length;
    },
    //提示targetCs 中可出的牌
    robot:function(targetCs)
    {
        this.clear();
        var targetCards = libddz.cards.create(targetCs);
        var line = this.getRobotLine(targetCards);
        if(line.length>0)
        {
            this.results.push(line);
            return true;
        }
        var d =[];
        var parse = targetCards;
        if(parse.card3.length>0)
        {
            d.push(parse.card3[0]);
            d.push(parse.card3[1]);
            d.push(parse.card3[2]);
            if(parse.card1.length>0)
            {
                if(parse.card1[0].face != libddz.Face.SMALL &&parse.card1[0].face != libddz.Face.BIG)
                {
                    d.push(parse.card1[0]);
                    this.results.push(d);
                    return true;
                }
            }
            if(parse.card2.length>0)
            {
                d.push(parse.card2[0]);
                d.push(parse.card2[1]);
                this.results.push(d);
            }
            return true;
        }
        else if(parse.card2.length>0)
        {
            d.push(parse.card2[0]);
            d.push(parse.card2[1]);
            this.results.push(d);
            return true;
        }
        else if(parse.card1.length == 2)
        {
            if(parse.card1[0].face == libddz.Face.SMALL &&parse.card1[1].face == libddz.Face.BIG)
            {
                d.push(parse.card1[0]);
            }
            d.push(parse.card1[1]);
            this.results.push(d);
            return true;
        }
        else if(parse.card1.length>0)
        {
            d.push(parse.card1[0]);
            this.results.push(d);
            return true;
        }
        else if(parse.card4.length>0)
        {
            d.push(parse.card1[0]);
            d.push(parse.card1[1]);
            d.push(parse.card1[2]);
            d.push(parse.card1[3]);
            this.results.push(d);
            return true;
        }
        return false;
    },
    //查找最长的顺
    getRobotLine:function(targetCards)
    {
        var line1 = this.getLineMax(1,targetCards.line1);
        var line2 = this.getLineMax(2,targetCards.line2);
        var line3 = this.getLineMax(3,targetCards.line3);
        if(line3.length>=line2.length&&line3.length>=line1.length&&line2.length>=6)
        {
            return line3;
        }
        if(line2.length>=line1.length&&line2.length>=line3.length&&line2.length>=6)
        {
            return line2;
        }
        if(line1.length>=line2.length&&line1.length>=line3.length&&line1.length>=5)
        {
            return line1;
        }
        return [];
    },
    //获取最长的顺
    getLineMax:function(type,targetCs)
    {
        var lastIndex =0;
        var lastNum =0;
        var lineNum =0;
        var temp=libddz.card.create(15);
        var flag =0;
        var i=0;
        for(;i<targetCs.length;i+=type)
        {
            if(targetCs[i].face >libddz.Face.ACE)
            {
                break;
            }
            if(targetCs[i].face -temp.face !=1)
            {
                if(lineNum>lastNum)
                {
                    lastIndex =i;
                    lastNum = lineNum;
                }
                flag =1;
                lineNum =0;
            }
            else
            {
                flag =0;
            }
            lineNum+=type;
            temp = targetCs[i];
        }
        if(flag ==0&&lineNum>lastNum)
        {
            lastIndex =i;
            lastNum = lineNum;
        }
        var ret=[];
        for(var i = lastIndex-lastNum;i<lastIndex;i++)
        {
            ret.push(targetCs[i]);
        }
        return ret;
    },
    endLaiZi:function(cards)
    {
        for(var i in cards)
        {
            if(cards[i].suit == libddz.Suit.LAIZI && cards[i].face >13)
            {
                cards[i].face -= 13;
                cards[i].parse(cards[i].face|0x40);
            }
        }
    }

};