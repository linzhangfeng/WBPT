...
var DDZCards =
{
    ...
    hasBomb:function(c)
    {
        ...
        {
            if(libddz.card.compareFace(this.handCardValues[i] ,c) == 0)
            {
                num++
            }
        }
    ...
    hasDiff:function(c,cs)
    {
        for(var i in cs)
        {
            if(libddz.card.compareFace(cs[i] ,c) != 0)
            {
                return true;
            }
        }
    ...
    checkCardUp:function()
    {
        ...
        if(!DDZModel.isMySeatID(DDZModel.lastSeatID))
        {
                ...
                ||type == libddz.CardsType.CARD_TYPE_THREE )//单张、对子、三张自动处理
            {
                ...
                if(!is&&cards1.length == 1 && type == libddz.CardsType.CARD_TYPE_ONE
                    &&this.hasBomb(cards1[0])&&!this.hasDiff(cards1[0], cards))
                {
                    is = true;
                }
                if(!is)
