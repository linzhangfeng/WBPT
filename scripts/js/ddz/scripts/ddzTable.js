...
//*************************** 按钮点击发送服务器处理 ************************************//
 ...
 //*************************** 服务器返回信息处理 ************************************//
    ...
    handlerTableInfoUC:function(val) {
        ...
        if (val.hasOwnProperty("piao_ratio")) {
            for (var i = 0; i < val["piao_ratio"].length; ++i) {
                DDZModel.piaoCoins.push(val["piao_ratio"][i]);
            }
        }
        ...
        if (val.hasOwnProperty("must_be_landlord")) {
            DDZModel.mustDz = val["must_be_landlord"];
        }
        ...
        //恢复公共牌
        if (DDZModel.state == DDZState.PREPLAY_ONE || DDZModel.state == DDZState.PREPLAY_TWO || DDZModel.state == DDZState.PIAO) {
            for (var i = 0; i < 3; ++i) {
        ...
        if (val.hasOwnProperty("players")) {
            ...
            for (var i = 0; i < len; ++i) {
                ...
                if(DDZModel.isOnVideo)
                {
                    ...
                    && DDZModel.state != DDZState.READY && DDZModel.state != DDZState.PIAO) {
    ...
    handlerMingPaiSuccBC:function(val)
    {
       ...
       // DDZTips.hideTime();
        ...
        DDZCards.showHandCards(pos,DDZCards.showCardValues[0].length);
