var DDZPlayerChat = cc.Node.extend({
    ctor: function () {
        this._super();
    },
    initChat: function (pos, content, flippedX, flippedY) {
        var imageView = new ccui.ImageView("ddz/icon_chat_020.png",1);
    ...
    initSound: function (pos, filepath, flippedX, flippedY, time) {
        var imageView = new ccui.ImageView("ddz/icon_chat_021.png",1);
        imageView.setScale9Enabled(true);
        imageView.setCapInsets(cc.rect(40, 25, 18, 13));
        imageView.setAnchorPoint(cc.p(0.0, 0.5));
        this.addChild(imageView);
        ...
        var text = new cc.Sprite(utils.spriteFrame("ddz/icon_chat_003.png"));
        text.setAnchorPoint(cc.p(0.0, 0.5));
        text.setScale(-0.2);
        this.addChild(text);

        var animation = utils.newAni1("ddz/icon_chat_00",[1,2,3],0.2,0);
