...
//*********************动态数据处理************************///
 ...
 //初始化联网
    ...
    updatePlayer:function(val)
    {
        ...
        if(val.hasOwnProperty("rob_ratio"))
        {
            player.rob_ratio = utils.getValInt(val,"rob_ratio");
        }
        else
        {
            player.rob_ratio = -1;
        ...
        if(val.hasOwnProperty("cards"))
        {
            ...
            for(var i = 0; i<len; i++)
            {
                ...
                player.spellcards.push(val["cards"][i]);
            }
        }


        player.is_niu = utils.getValBool(val,"is_niu");
        if(val.hasOwnProperty("max_card"))
        {
            player.max_card = utils.getValInt(val,"max_card");
        }
        else
        {
            player.max_card = 0;
 ...
 //*********************联网协议处理************************///
    ...
    sendLogin:function()
    {
        ...
        d.data["vid"]  = YZDNModel.roomData["gameRoom"]["gameId"];
        this.sendData(d.toStr());
    },
