...
def main():
    ...
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..','cocos2d'))
    cocos_root = os.path.abspath(project_root)
    class_root = os.path.abspath(os.path.join(project_root, '../Classes'))
    ...
    config.set('DEFAULT', 'classroot', class_root)
    ...
    try:
        ...
        tojs_root = '%s/../jsbind' % project_root
        output_dir = '%s/jsbind' % class_root

        cmd_args = {'wb.ini': ('wb', 'js_wb_auto'),
                    }
        target = 'spidermonkey'
